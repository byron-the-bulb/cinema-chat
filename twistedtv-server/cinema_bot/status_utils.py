...
class StatusUpdater:
    ...
    async def needs_help(self, phase, needs_help=True):
        """
        Send a help request notification to the frontend.
        ...
        """
        logger.info(f"needs_help method called with phase={phase}, needs_help={needs_help}")
        logger.info(f"Using station name: {self.station_name}")
        logger.info(f"RTVI processor initialized: {self.rtvi is not None}")
        logger.info(f"Identifier: {self.identifier}")
        
        if not self.rtvi:
            logger.error("StatusUpdater not initialized with RTVI processor - cannot send help request")
            return False
        ...
        try:
            logger.info(f"Creating help request frame with data: {data}")
            status_frame = RTVIServerMessageFrame(data)
            logger.info(f"Frame created: {status_frame}")
            logger.info(f"Frame data: {status_frame.data if hasattr(status_frame, 'data') else 'No data attribute'}")
            logger.info(f"Pushing help request frame to RTVI processor")
            logger.info(f"RTVI processor type: {type(self.rtvi).__name__}")
            logger.info(f"RTVI processor methods: {[m for m in dir(self.rtvi) if not m.startswith('_') and callable(getattr(self.rtvi, m))]}")
            
            # Verify push_frame method exists
            if not hasattr(self.rtvi, 'push_frame'):
                logger.error(f"RTVI processor does not have push_frame method!")
                return False
                
            # Try pushing the frame with more detailed error handling
            logger.info(f"About to call push_frame on RTVI processor")
            push_result = await self.rtvi.push_frame(status_frame)
            logger.info(f"Push frame result: {push_result}")
            logger.info(f"Successfully sent help request for station: {self.station_name}, phase: {phase}, needs_help={needs_help}")
            return True
        except Exception as e:
            logger.error(f"Error sending needs_help event: {e}")
            import traceback
            logger.error(f"Traceback: {traceback.format_exc()}")
            logger.error(f"RTVI object: {self.rtvi}")
            logger.error(f"RTVI state: {vars(self.rtvi) if self.rtvi else 'None'}")
            return False
