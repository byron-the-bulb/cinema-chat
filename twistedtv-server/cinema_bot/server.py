...
async def start_agent(request: Request):
    ...
    # Add all NVIDIA CUDA library paths for CUDA support
    import site
    import glob
    packages = site.getsitepackages()[0]
    nvidia_libs = glob.glob(os.path.join(packages, 'nvidia/*/lib'))
    print(f"[DEBUG] Found {len(nvidia_libs)} NVIDIA library directories: {nvidia_libs}")
    if nvidia_libs:
        current_ld_path = env.get('LD_LIBRARY_PATH', '')
        lib_paths = ':'.join(nvidia_libs)
        env['LD_LIBRARY_PATH'] = f"{lib_paths}:{current_ld_path}" if current_ld_path else lib_paths
        print(f"[DEBUG] Set LD_LIBRARY_PATH={env['LD_LIBRARY_PATH']}")
    else:
        print(f"[DEBUG] No NVIDIA libraries found in {packages}")

    try:
        # Capture stdout/stderr to a log file for debugging
        log_file = open(f"/tmp/bot_{identifier[:8]}.log", "w")
        proc = subprocess.Popen(
            [sys.executable, "-m", "cinema_bot", "-u", room_url, "-t", token, "-i", identifier],
            bufsize=1,
            cwd=src_dir,
            env=env,
            stdout=log_file,
...
async def rtvi_connect(request: Request) -> Dict[Any, Any]:
    ...
    cmd_args = [sys.executable, "-m", "cinema_bot", "-u", room_url, "-t", token, "-i", identifier]
