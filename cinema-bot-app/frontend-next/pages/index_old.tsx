...
const DEFAULT_VIDEO_SERVICE_URL = process.env.NEXT_PUBLIC_VIDEO_SERVICE_URL || 'http://192.168.1.201:5000';
...
const createClient = (config: ClientConfig = defaultTtsConfig) => {
  try {
    // Extract TTS config, station name, audio device ID, and server URL
    const { stationName, audioDeviceId, serverUrl, ...ttsConfig } = config;

    // Use provided server URL or fall back to API_URL
...
export default function Home() {
  ...
  const [serverUrl, setServerUrl] = useState(API_URL); // Server URL (configurable)
  ...
  const setupEventHandlers = useCallback((client: RTVIClient) => {
    ...
    const handleConnected = async () => {
      ...
      // Notify Pi client about the room (if running on Pi)
      try {
        // Get room URL from transport
        const transport = (client as any).transport;
        const roomUrl = transport?._daily?.room()?.url || transport?._options?.url;

        if (roomUrl) {
          await fetch('/api/notify_pi', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ roomUrl })
          });
          console.log('Notified Pi client about room:', roomUrl);
        }
      } catch (error) {
    ...
    const handleServerMessage = (event: any) => {
      ...
      if (event) {
        // Check for video playback command
        if (event.type === 'video-playback-command' && event.action === 'play') {
          console.log('Video playback command received:', event);
          ...
          .then(data => {
            console.log('Video playback started:', data);
            addChatMessage(`Playing: ${event.video_path} (${event.start}s - ${event.end}s)`, 'guide');
          })
          .catch(error => {
            console.error('Error starting video playback:', error);
            addChatMessage(`Error playing video: ${error.message}`, 'system');
          });
        }
  ...
  const handleStartConnection = useCallback(() => {
    ...
    // Create a new client with the current TTS configuration, station name, audio device, and server URL
  ...
  }, [ttsConfig, setupEventHandlers, stationName, selectedAudioDeviceId, serverUrl]); // Add serverUrl to dependencies
        ...
        */}
        ...
        <div className={styles.stationNameContainer}>
          ...
          <input
            ...
            className={styles.stationNameInput}
            disabled={isConnected || isConnecting}
            placeholder="http://172.28.172.5:3000/api"
          />
        </div>
